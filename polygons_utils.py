...
class PolygonApp:
    def __init__(self, root):
        ...
        # Load polygons from MongoDB after a short delay to ensure video feed is ready
    ...
    def update_video(self):
        while self.video_running:
            ...
            if ret:
                ...
                for person_id, (center_x, center_y, polygon_index, missed_frames) in list(self.person_tracker.items()):
                    if person_id not in used_previous_ids:
                        ...
                        if missed_frames >= self.max_missed_frames and polygon_index is not None:
                            self.log_action(f"Person-{person_id} left Polygon-{polygon_index}")
                ...
                for person_id, (center_x, center_y, prev_polygon_index, missed_frames) in self.person_tracker.items():
                    ...
                    if prev_polygon_index is None and inside_polygon_index is not None:
                        self.log_action(f"Person-{person_id} entered Polygon-{inside_polygon_index}")
                        self.mongo_handler.save_event_log(person_id, inside_polygon_index, "enter")
                        self.person_tracker[person_id] = (center_x, center_y, inside_polygon_index, 0)
                    elif prev_polygon_index is not None and inside_polygon_index is None and missed_frames == 0:
                        self.log_action(f"Person-{person_id} left Polygon-{prev_polygon_index}")
                        self.mongo_handler.save_event_log(person_id, prev_polygon_index, "leave")
                        self.person_tracker[person_id] = (center_x, center_y, None, 0)
